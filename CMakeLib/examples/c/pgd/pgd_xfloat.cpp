...
//
//	std::string clockname = "timeprofile_floatx.txt";
//	std::string xkname = "xk_floatx.dat";
//	std::string errorrecordname = "error_record_floatx.dat";
//	std::string errorhistname = "error_hist_floatx.dat";
//	std::string figurename = "ProximalGradientDecent_floatx.png";
//
//#ifdef TIME_PROFILE
//	clock_t start = clock();
...
void PROXIMAL_GRADIENT_DECENT_XFLOAT2(fptx2 Amatrix_c[DIAG][DIAG],
	...
	TimeProfile.open(clockname);
   	...
   	fptx2 grad_g[DIAG];
   	fptx2 ones_vec[DIAG];
   	std::vector<double> error_hist(ITER_MAX, 0.0);
   	std::vector<double> error_record(ITER_MAX, 0.0);
	...
	while( k<ITER_MAX ){
		...
		error = norm1;
#ifdef RECORD_RESULT
		error_hist.at(k) = (double)(norm1 / norm2);
		error_record.at(k) = (double)error;
	...
	for(int i=0; i<ITER_MAX; i++){
		if( error_hist.at(i)>0 ){
			x.push_back(i);
			y.push_back((double)error_hist.at(i));
			resultfile << "  " << i << "," << (double)error_hist.at(i) << " \n";
			resultfile1 << "  " << i << "," << (double)error_record.at(i) << " \n";
			std::vector<double> xkvec = x_k_record.at(i);
			for(int j=0;j<DIAG-1;j++)
				resultfile2 << (double) xkvec.at(j) << ",";
			resultfile2 << (double) xkvec.at(DIAG-1) << "\n";
		}
	}
	resultfile.close();
	resultfile1.close();
...
void PROXIMAL_GRADIENT_DECENT_XFLOAT2(fptx2 **Amatrix_c,
	...
	std::string clockname = "timeprofile_gfptx2.txt";
	std::string xkname = "xk_gfptx2.dat";
	std::string errorrecordname = "error_record_gfptx2.dat";
	std::string errorhistname = "error_hist_gfptx2.dat";
	std::string figurename = "ProximalGradientDecent_gfptx2.png";
	...
	TimeProfile.open(clockname);
   	...
   	fptx2 *grad_g;
   	fptx2 *ones_vec;
   	x_k_vec = (fptx2*) malloc(sizeof(fptx2)*DIAG);
   	x_k_plus1_vec = (fptx2*) malloc(sizeof(fptx2)*DIAG);
   	grad_g = (fptx2*) malloc(sizeof(fptx2)*DIAG);
   	ones_vec = (fptx2*) malloc(sizeof(fptx2)*DIAG);
	...
	while( k<ITER_MAX ){
		...
		// Compute gradient
		fptx2 *tmp_mul_vec1;
		tmp_mul_vec1 = (fptx2*) malloc(sizeof(fptx2)*DIAG);
...
//	   	std::cout << __FILE__ << __LINE__ << std::endl;
		...
		fptx2 *new_point;
		fptx2 *tmp_mul_vec2;
		new_point = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		tmp_mul_vec2 = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		Float_Point_Algebra_obj.VEC_SCALAR_MUL<fptx2, DIAG>( grad_g, 1/L_c,
									 tmp_mul_vec2 );
		Float_Point_Algebra_obj.VEC_SUB<fptx2, DIAG>( x_k_vec, tmp_mul_vec2,
								new_point );
		...
		fptx2 *rndnoise;
		fptx2 *a_ones_vec;
		fptx2 *minus_a_ones_vec;
		rndnoise = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		a_ones_vec = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		minus_a_ones_vec = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		Float_Point_Algebra_obj.ONES_VEC<fptx2, DIAG>( a_ones_vec );
		Float_Point_Algebra_obj.ONES_VEC<fptx2, DIAG>( minus_a_ones_vec );
		fptx2 *tmp_min;
		fptx2 *tmp_max;
		fptx2 *tmp_mul_vec3;
		tmp_min = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		tmp_max = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		tmp_mul_vec3 = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		Float_Point_Algebra_obj.RND_VEC<fptx2, DIAG>( rndnoise );
		Float_Point_Algebra_obj.VEC_SCALAR_MIN<fptx2, DIAG>(new_point, BOX_CONST, tmp_min);
		Float_Point_Algebra_obj.VEC_SCALAR_MAX<fptx2, DIAG>(tmp_min, -BOX_CONST, tmp_max);
		Float_Point_Algebra_obj.VEC_SCALAR_MUL<fptx2, DIAG>( rndnoise, error_std,
		...
		fptx2 norm1, norm2;
		fptx2 *tmp_sub_vec;
		tmp_sub_vec = (fptx2*) malloc(sizeof(fptx2)*DIAG);
		Float_Point_Algebra_obj.VEC_SUB<fptx2, DIAG>( x_k_vec, x_k_plus1_vec,
								tmp_sub_vec);
		Float_Point_Algebra_obj.VEC_NORM<fptx2, DIAG>(tmp_sub_vec, norm1);
		Float_Point_Algebra_obj.VEC_NORM<fptx2, DIAG>(x_k_vec, norm2);
