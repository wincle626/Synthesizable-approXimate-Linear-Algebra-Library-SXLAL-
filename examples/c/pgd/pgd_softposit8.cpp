...
void printscalar(posit8_t scalar){
	std::cout << convertP8ToDouble(scalar) << std::endl;
	std::cout << std::endl;
}

void printvector(posit8_t *vector, int length){
	for(int i=0;i<length;i++){
		std::cout << convertP8ToDouble(vector[i]) << std::endl;
	}
	std::cout << std::endl;
}

void printmatrix(posit8_t **matrix, int width, int height){
	for(int i=0;i<height;i++){
		for(int j=0;j<width;j++){
			std::cout << convertP8ToDouble(matrix[i][j])
					  << ",";
		}
		std::cout << std::endl;
	}
