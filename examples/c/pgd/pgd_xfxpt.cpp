...
void PROXIMAL_GRADIENT_DECENT_XFXPT(DATA_IN_T **Amatrix_c,
	...
	TimeProfile.open(clockname);
   	...
   	DATA_IN_T *x_k_vec;
   	DATA_IN_T *x_k_plus1_vec;
   	DATA_IN_T *grad_g;
   	DATA_IN_T *ones_vec;
   	x_k_vec = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
   	x_k_plus1_vec = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
   	grad_g = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
   	ones_vec = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
   	std::vector<float> error_record(ITER_MAX, 0);
   	std::vector<float> error_hist(ITER_MAX, 0);
   	std::vector<std::vector<float>> x_k_record;
	...
	while( k<ITER_MAX ){
		...
		// Compute gradient
		DATA_IN_T *tmp_mul_vec1;
		tmp_mul_vec1 = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		Xilinx_Fixed_Point_Algebra_obj.MAT_VEC_MUL<DATA_IN_T, DIAG, DIAG>(
				Amatrix_c, x_k_vec,	tmp_mul_vec1 );
#ifdef DEBUG_ITER
		for(int i=0;i<DIAG;i++)
		...
		// new decent point
		DATA_IN_T *new_point;
		DATA_IN_T *tmp_mul_vec2;
		new_point = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		tmp_mul_vec2 = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		Xilinx_Fixed_Point_Algebra_obj.VEC_SCALAR_MUL<DATA_IN_T, DIAG>( grad_g, factor,
									 tmp_mul_vec2 );
#ifdef DEBUG_ITER
		for(int i=0;i<DIAG;i++)
		...
		// Proximal projection
		DATA_IN_T *rndnoise;
		DATA_IN_T *a_ones_vec;
		DATA_IN_T *minus_a_ones_vec;
		rndnoise = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		a_ones_vec = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		minus_a_ones_vec = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		Xilinx_Fixed_Point_Algebra_obj.ONES_VEC<DATA_IN_T, DIAG>( a_ones_vec );
		Xilinx_Fixed_Point_Algebra_obj.ONES_VEC<DATA_IN_T, DIAG>( minus_a_ones_vec );
		DATA_IN_T *tmp_min;
		DATA_IN_T *tmp_max;
		DATA_IN_T *tmp_mul_vec3;
		tmp_min = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		tmp_max = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		tmp_mul_vec3 = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		Xilinx_Fixed_Point_Algebra_obj.RND_VEC<DATA_IN_T, DIAG>( rndnoise );
		Xilinx_Fixed_Point_Algebra_obj.VEC_SCALAR_MIN<DATA_IN_T, DIAG>(new_point, BOX_CONST, tmp_min);
		Xilinx_Fixed_Point_Algebra_obj.VEC_SCALAR_MAX<DATA_IN_T, DIAG>(tmp_min, -BOX_CONST, tmp_max);
		Xilinx_Fixed_Point_Algebra_obj.VEC_SCALAR_MUL<DATA_IN_T, DIAG>( rndnoise, error_std,
		...
		// check early termination constraint
		DATA_IN_T norm1, norm2;
		DATA_IN_T *tmp_sub_vec;
		tmp_sub_vec = (DATA_IN_T*) malloc(sizeof(DATA_IN_T)*DIAG);
		Xilinx_Fixed_Point_Algebra_obj.VEC_SUB<DATA_IN_T, DIAG>( x_k_vec, x_k_plus1_vec,
								tmp_sub_vec);
#ifdef DEBUG_ITER
		for(int i=0;i<DIAG;i++)
