...
void printscalar(posit16_t scalar){
	std::cout << convertP16ToDouble(scalar) << std::endl;
	std::cout << std::endl;
}

void printvector(posit16_t *vector, int length){
	for(int i=0;i<length;i++){
		std::cout << convertP16ToDouble(vector[i]) << std::endl;
	}
	std::cout << std::endl;
}

void printmatrix(posit16_t **matrix, int width, int height){
	for(int i=0;i<height;i++){
		for(int j=0;j<width;j++){
			std::cout << convertP16ToDouble(matrix[i][j])
					  << ",";
		}
		std::cout << std::endl;
	}
