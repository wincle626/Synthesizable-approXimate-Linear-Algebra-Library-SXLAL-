...
void printscalar(posit32_t scalar){
	std::cout << convertP32ToDouble(scalar) << std::endl;
	std::cout << std::endl;
}

void printvector(posit32_t *vector, int length){
	for(int i=0;i<length;i++){
		std::cout << convertP32ToDouble(vector[i]) << std::endl;
	}
	std::cout << std::endl;
}

void printmatrix(posit32_t **matrix, int width, int height){
	for(int i=0;i<height;i++){
		for(int j=0;j<width;j++){
			std::cout << convertP32ToDouble(matrix[i][j])
					  << ",";
		}
		std::cout << std::endl;
	}
