...
void PROXIMAL_GRADIENT_DECENT_GDOUBLE(double **Amatrix_c,
	...
	TimeProfile.open(clockname);
   	...
   	double *x_k_vec;
   	double *x_k_plus1_vec;
   	double *grad_g;
   	double *ones_vec;
   	x_k_vec = (double*) malloc(sizeof(double)*DIAG);
   	x_k_plus1_vec = (double*) malloc(sizeof(double)*DIAG);
   	grad_g = (double*) malloc(sizeof(double)*DIAG);
   	ones_vec = (double*) malloc(sizeof(double)*DIAG);
   	std::vector<double> error_hist(ITER_MAX, 0);
   	std::vector<double> error_record(ITER_MAX, 0);
   	std::vector<std::vector<double>> x_k_record;
	...
	while( k<ITER_MAX ){
		...
		// Compute gradient
		double *tmp_mul_vec1;
		tmp_mul_vec1 = (double*) malloc(sizeof(double)*DIAG);
...
//	   	std::cout << __FILE__ << __LINE__ << std::endl;
		...
		double *new_point;
		double *tmp_mul_vec2;
		new_point = (double*) malloc(sizeof(double)*DIAG);
		tmp_mul_vec2 = (double*) malloc(sizeof(double)*DIAG);
		Float_Point_Algebra_obj.VEC_SCALAR_MUL<double, DIAG>( grad_g, 1/L_c,
									 tmp_mul_vec2 );
		Float_Point_Algebra_obj.VEC_SUB<double, DIAG>( x_k_vec, tmp_mul_vec2,
								new_point );
		...
		double *rndnoise;
		double *a_ones_vec;
		double *minus_a_ones_vec;
		rndnoise = (double*) malloc(sizeof(double)*DIAG);
		a_ones_vec = (double*) malloc(sizeof(double)*DIAG);
		minus_a_ones_vec = (double*) malloc(sizeof(double)*DIAG);
		Float_Point_Algebra_obj.ONES_VEC<double, DIAG>( a_ones_vec );
		Float_Point_Algebra_obj.ONES_VEC<double, DIAG>( minus_a_ones_vec );
		double *tmp_min;
		double *tmp_max;
		double *tmp_mul_vec3;
		tmp_min = (double*) malloc(sizeof(double)*DIAG);
		tmp_max = (double*) malloc(sizeof(double)*DIAG);
		tmp_mul_vec3 = (double*) malloc(sizeof(double)*DIAG);
		Float_Point_Algebra_obj.RND_VEC<double, DIAG>( rndnoise );
		Float_Point_Algebra_obj.VEC_SCALAR_MIN<double, DIAG>(new_point, BOX_CONST, tmp_min);
		Float_Point_Algebra_obj.VEC_SCALAR_MAX<double, DIAG>(tmp_min, -BOX_CONST, tmp_max);
		Float_Point_Algebra_obj.VEC_SCALAR_MUL<double, DIAG>( rndnoise, error_std,
		...
		double norm1, norm2;
		double *tmp_sub_vec;
		tmp_sub_vec = (double*) malloc(sizeof(double)*DIAG);
		Float_Point_Algebra_obj.VEC_SUB<double, DIAG>( x_k_vec, x_k_plus1_vec,
								tmp_sub_vec);
		Float_Point_Algebra_obj.VEC_NORM<double, DIAG>(tmp_sub_vec, norm1);
		Float_Point_Algebra_obj.VEC_NORM<double, DIAG>(x_k_vec, norm2);
